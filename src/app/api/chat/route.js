...
export async function POST(req) {
  try {
    ...
    const hasName = messages.some(m => m.role === 'user' && m.content.split(' ').length >= 2);
    const services = ['equipment', 'location', 'production', 'soundstage', 'studio', 'crew', 'talent'];
    const hasService = services.some(s => fullConversation.toLowerCase().includes(s));
      ...
      try {
        await resend.emails.send({
          ...
          html: `
            ...
            <pre style="background:#111;color:#fff;padding:16px;border-radius:8px;">${messages.slice(-6).map(m => `[${m.role.toUpperCase()}]: ${m.content}`).join('\n\n')}</pre>
